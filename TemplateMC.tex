...
\newcommand{\mydate}{\today} % 3. 在这里定义日期，\today 会自动显示当前日期，也可以写成 {2026年5月}

\begin{frame}
  \begin{tikzpicture}[remember picture, overlay]
    % 2. 蓝色标题栏背景（将 minimum height 从 0.5 改为了 0.4，上下距离就缩短了）
    ...
    % 1. 加上校徽（放置在蓝色画布的左上角，高设为 1.2cm，可根据需要调节）
    \node[anchor=north west, xshift=0.5cm, yshift=-0.4cm] at (current page.north west) {
        \includegraphics[height=1.2cm]{校徽.png}
    };
    
    % 标题（因为蓝色背景变小了，所以将标题的 yshift 从 -0.265 调整到了 -0.21，使其依然居中在蓝色区域内）
    \node[text=white, 
